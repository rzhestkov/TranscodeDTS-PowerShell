...
Если что-то идет не так, можно читать текстовые файлы диагностики, которые процессе рождает **ffmpeg.exe**. Для этого нужно поставить комментарий `#` перед строкой `$delTxtLogs=$true`.
