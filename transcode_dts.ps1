...
foreach ($FileSpec in $FileList) {
    ...
    if (-not (IsFileExists($FileSpec))) {
        # почему-то его не оказалось на месте
        Continue 
    }
    ...
    foreach ($fline in Get-Content $infoFileName) {
        ...
        if ($fline -match 'Stream #0:') {
            ...
            elseif ($fline -match ': Subtitle: ') {
                #это субтитры, копируем их
    ...
    if ($numToTranscode -gt 0) {
        #есть дорожки для конвертации. обработка файла
        ...
        #------------------------------------------------------------------
        if (IsFileExists($oFileName)) {
            #выходной файл есть. считаем, что перекодирование завершено
            ...
            #что-то пошло не так. нет выходного файла
